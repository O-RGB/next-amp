# NextSona Go Engine / Chrome Web Store Build Isolation Plan
...
- ห้ามลบโฟลเดอร์ `nextsona-engine-go/`
...
- [x] Go source และ binary ถูกเก็บแยกใน `nextsona-engine-go/`
...
dist/nextsona-extension-store.zip
```
...
```text
dist/nextsona-extension-go-dev/
dist/nextsona-extension-go-dev.zip
nextsona-engine-go/nextsona-engine
nextsona-engine-go/nextsona-engine.exe
```
...
```json
{
  ...
  "build:go": "bash nextsona-engine-go/build.sh"
}
```
...
- [x] เรียก `nextsona-engine-go/build.sh` เฉพาะ `go-dev`
- [x] ห้าม Store build อ่านหรือ copy binary จาก `nextsona-engine-go/`
...
- [ ] ติดตั้งใหม่จาก `nextsona-extension-store/`
...
- `NEXTSONA-PUBLIC-WEB-DEPLOY-PLAN.md`
