...
- [x] Web exact candidate รวม crop → Transpose → Reshape → Sigmoid เป็น output head `[2,32,1024]` สำหรับ Smooth/Detail ร่วมกัน (active + next tail) — implement แล้วแต่ปิด production หลังพบ vocal artifact บน provider จริง; รอ WebGPU/WebGL listening gate
- [x] GO output head แบบ runtime rewrite เหลือ `[1,1024,32,2]` (active + next tail) — implement แล้วแต่ปิด production หลังพบ vocal artifact บน provider จริง; overlap consensus ยังปิด
- [x] Web output head ลดผลลัพธ์จาก 64 เป็น shared 32 frames — implement แล้วแต่ปิด production เพื่อคืน full-output quality baseline
