# NextStudio Model Builder Plan

เป้าหมายของโฟลเดอร์นี้คือสร้างโมเดลของ NextStudio ใหม่จาก
...
├── NEXTSTUDIO-MODEL-BUILDER-PLAN.md
├── README.md
├── Dockerfile
├── requirements.lock
├── package.json
...
## ตำแหน่งที่ NextStudio ใช้งานจริง
...
```text
nextstudio-model-builder/dist/mgm-main-v4/tfjs/
```
...
```text
nextstudio-model-builder/dist/mgm-main-v4/onnx/model.onnx
```

ไปยัง:

```text
nextstudio-engine-go/model.onnx
```

หลังจากนั้นจึงเรียก packer เดิมให้สร้าง:

```text
nextstudio-engine-go/assets/model.enc
```
...
`next-amp-extension/model/` หรือรับ model URL จาก environment แล้วนำ binary
...
{
  "scripts": {
    "model:fetch": "bash nextstudio-model-builder/scripts/build.sh --fetch-only",
    "model:build": "bash nextstudio-model-builder/scripts/build.sh",
    "model:verify": "bash nextstudio-model-builder/scripts/verify.sh",
    "model:deploy": "bash nextstudio-model-builder/scripts/deploy.sh",
...
- `npm run model:build:deploy` เป็นคำสั่งเดียวสำหรับสร้าง ตรวจ และ copy ไปยัง
  NextStudio ทั้ง Web/Extension และ Go
...
- [x] ตั้ง input tensor ชื่อ `input` และ output ให้ NextStudio หาได้แน่นอน
...
- [x] ให้ผลลัพธ์เป็น shard เดียวเพื่อคงรูปแบบการโหลดที่ NextStudio ใช้อยู่
...
- [x] copy ไป `nextstudio-engine-go/model.onnx` หลัง verify เท่านั้น
