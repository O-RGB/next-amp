...
export const NEXTSTUDIO_OPTIMIZATION_METADATA = "nextstudioModelOptimization";

/**
 * Build-time TensorFlow.js graph optimizer.
 *
...
function appendExactOutputHead(artifacts, nodes, options) {
  ...
  const prefix = "NextStudio/optimized_output_head";
  const needsOutputCrop = !canApplySourceCrop || requestedHeadStart !== 0 || roiFrames !== frames;
  const constants = [
    ...(needsOutputCrop ? [
  ...
  const allConstants = [
    ...decoderConstants.map(item => ({ ...item, prefix: "NextStudio/roi_decoder_layer" })),
    ...roiConstants.map(item => ({ ...item, prefix: "NextStudio/roi_decoder_input" })),
  ...
  const outputSpecs = new Map(
    extraSpecs.slice(decoderSpecCount + roiSpecCount).map(spec => [spec.name.slice(prefix.length + 1), spec.name])
  );
  const begin = outputSpecs.get("begin");
  const end = outputSpecs.get("end");
  const strides = outputSpecs.get("strides");
  const transposePerm = outputSpecs.get("transpose_perm");
  const reshape = outputSpecs.get("reshape_shape");
  const decoderPrefix = "NextStudio/roi_decoder_layer";
  const roiPrefix = "NextStudio/roi_decoder_input";
  ...
  const headNodes = [
    ...
    }] : []),
    {
      name: transposeName,
      op: "Transpose",
      input: [needsOutputCrop ? cropName : outputNode.input[0], transposePerm],
...
/**
 * Convert the verified converter output into the graph shipped by NextStudio.
 *
 * The heavy graph inspection and rewriting happens here, once during build,
 * instead of on every user's device. The fallback topology is embedded as
 * metadata and shares the same weights, so a driver that rejects native
 * dilation/explicit padding can still load the conservative converter graph
 * without downloading another 15 MB shard.
 */
export function buildRuntimeTfjsArtifacts(modelJson, weightBytes) {
  const weightSpecs = modelJson.weightsManifest.flatMap(group => group.weights || []);
  const baselineArtifacts = {
    ...modelJson,
    weightSpecs,
    weightData: asArrayBuffer(weightBytes)
  };
  const result = optimizeVocalModelArtifacts(baselineArtifacts, {
    optimizeGraph: true,
    outputHead: {
      start: 34,
      frames: 15,
      bins: 1024,
      headStart: 0,
      // Only crop the input of the final frame-independent 1x1 projection.
      // The entire decoder and full 64-frame receptive context are retained.
      sourceCrop: { start: 34, frames: 15, inputFrames: 64, bins: 1024 }
    }
  });

  if (result.foldedCount !== 12 || result.explicitPadCount !== 16) {
    throw new Error(
      `Unexpected graph contract: folded=${result.foldedCount}, explicitPads=${result.explicitPadCount}`
    );
  }
  if (!result.outputHead || result.outputHead.decoderLayerRoi !== null ||
      result.outputHead.start !== 34 || result.outputHead.frames !== 15) {
    throw new Error("Safe ECO projection-only output head was not applied");
  }

  const optimized = result.artifacts;
  const groups = modelJson.weightsManifest || [];
  if (groups.length !== 1) {
    throw new Error(`Expected one TFJS weight group, found ${groups.length}`);
  }
  const baselineWeightNames = new Set(weightSpecs.map(spec => spec.name));
  const addedWeightNames = new Set(
    optimized.weightSpecs.filter(spec => !baselineWeightNames.has(spec.name)).map(spec => spec.name)
  );
  const fallbackConstNodes = optimized.modelTopology.node.filter(
    node => node.op === "Const" && addedWeightNames.has(node.name)
  );
  if (fallbackConstNodes.length !== addedWeightNames.size) {
    throw new Error("Could not attach every optimized constant to the compatibility graph");
  }
  // Keep build-added constants as disconnected nodes in the fallback graph.
  // TensorFlow.js then owns/disposes all decoded weights even when the driver
  // selects this topology; disconnected constants execute no kernels.
  const fallbackModelTopology = {
    ...modelJson.modelTopology,
    node: [...modelJson.modelTopology.node, ...fallbackConstNodes]
  };
  const metadata = {
    version: 1,
    foldedCount: result.foldedCount,
    explicitPadCount: result.explicitPadCount,
    outputHead: result.outputHead,
    fallback: {
      modelTopology: fallbackModelTopology,
      signature: modelJson.signature
    }
  };
  const runtimeJson = {
    ...modelJson,
    modelTopology: optimized.modelTopology,
    signature: optimized.signature,
    userDefinedMetadata: {
      ...(modelJson.userDefinedMetadata || {}),
      [NEXTSTUDIO_OPTIMIZATION_METADATA]: metadata
    },
    weightsManifest: [{
      ...groups[0],
      weights: optimized.weightSpecs
    }]
  };

  return {
    modelJson: runtimeJson,
    weightData: Buffer.from(optimized.weightData),
    metadata
  };
}

export function optimizeTfjsFiles(modelPath, weightsPath) {
  const modelJson = JSON.parse(fs.readFileSync(modelPath, "utf8"));
  const weightBytes = fs.readFileSync(weightsPath);
  const optimized = buildRuntimeTfjsArtifacts(modelJson, weightBytes);
  fs.writeFileSync(modelPath, JSON.stringify(optimized.modelJson));
  fs.writeFileSync(weightsPath, optimized.weightData);
  return optimized.metadata;
}

const invokedPath = process.argv[1] ? path.resolve(process.argv[1]) : "";
if (invokedPath === fileURLToPath(import.meta.url)) {
  const [, , modelPath, weightsPath] = process.argv;
  if (!modelPath || !weightsPath) {
    console.error("Usage: optimize_tfjs_graph.mjs <model.json> <weights.bin>");
    process.exit(2);
  }
  const metadata = optimizeTfjsFiles(modelPath, weightsPath);
  console.log(JSON.stringify({
    status: "optimized",
    version: metadata.version,
    foldedCount: metadata.foldedCount,
    explicitPadCount: metadata.explicitPadCount,
    outputHead: metadata.outputHead
  }, null, 2));
}
