...
void stft_forward(int num_frames) {
    ...
    int p = 0;
    for (int k = 0; k < NUM_BINS; k++) {
        // Write into the slots that just expired from the logical window.
        // This removes the per-bin 48-frame memmove from every chunk.
        for (int f = 0; f < num_frames; f++) {
...
void stft_prepare_norm_input(float inv_max) {
    ...
    for (int k = 0; k < NUM_BINS; k++) {
        ...
        for (int i = 0; i < first_vector_floats; i += 4) {
            ...
            vst1q_f32(&out[i], vmulq_f32(v, vinv));
        }
    ...
    for (int k = 0; k < NUM_BINS; k++) {
        ...
        for (int i = 0; i < first_vector_floats; i += 4) {
            ...
            _mm_storeu_ps(&out[i], _mm_mul_ps(v, vinv));
        }
        for (int i = first_vector_floats; i < first_floats; i++) {
            out[i] = src[i] * inv_max;
        }
