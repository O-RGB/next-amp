# NextSona Public Web Deployment Plan
...
สร้างเว็บไซต์ static แยกจาก Extension และ Web Player เพื่อใช้เป็นเว็บไซต์ภายนอกอย่างเป็นทางการของ NextSona โดยมีหน้าหลัก 3 ส่วน:
...
โฟลเดอร์นี้ต้องไม่ import ไฟล์โดยตรงจาก `nextsona-extension/` หรือ root Web Player เพื่อให้ deploy แยกได้จริงและไม่เผลอนำ source/model ของ Extension ขึ้นเว็บ
...
- [x] Hero section แสดงชื่อ `NextSona - Pitch Shifter, AI Vocal & Video Sync`
...
- [x] แสดง NextSona branding และชื่อ session แบบไม่เปิดเผย URL เต็ม
...
- [x] ทำให้ `nextsona-public-site/` เป็น static deployment folder ที่ deploy ได้โดยตรงโดยไม่ต้อง build
- [x] ยังคง output สำรองใน `dist/nextsona-public-site/` สำหรับกรณีต้องการ inventory/verification
...
- สร้าง `nextsona-public-site/` ตามแผน
...
- `/` อธิบาย NextSona และฟีเจอร์จริงได้ครบโดยไม่โฆษณาเกินจริง
