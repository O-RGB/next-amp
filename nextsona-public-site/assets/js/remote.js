(function () {
  ...
  const protocol = window.NextSonaRemoteProtocol || window.NextStudioRemoteProtocol;
  ...
  const translations = {
    en: {
      ...
      "remote.retry": "Retry",
      "remote.donate": "Support NextSona",
    ...
    th: {
      ...
      "remote.donate": "สนับสนุน NextSona",
  ...
  function connect() {
    ...
    if (!fragment.host || !fragment.token) {
      ...
      showReconnect(true, "Open this page from the Remote link generated by NextSona.");
      return;
    }
  ...
  try {
    ...
    if (!initialLanguage) {
      initialLanguage = localStorage.getItem(LEGACY_LANGUAGE_KEY) || "en";
      if (initialLanguage === "en" || initialLanguage === "th") localStorage.setItem(LANGUAGE_KEY, initialLanguage);
    }
  } catch (_) {}
