...
// Run with:
//
//	NEXTSONA_RUN_COREML_BENCH=1 go test -run TestCoreMLComputeUnits -v
//
...
func TestCoreMLComputeUnits(t *testing.T) {
	...
	if compatEnv("NEXTSONA_RUN_COREML_BENCH", "NEXTSTUDIO_RUN_COREML_BENCH") != "1" {
		t.Skip("set NEXTSONA_RUN_COREML_BENCH=1 to run the opt-in provider benchmark")
	}
