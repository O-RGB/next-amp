...
// getNextSonaCacheDir returns the OS-specific cache directory for NextSona
func getNextSonaCacheDir() string {
	switch runtime.GOOS {
	case "windows":
		if localAppData := os.Getenv("LOCALAPPDATA"); localAppData != "" {
			return filepath.Join(localAppData, "NextSona")
		}
		return filepath.Join(os.TempDir(), "NextSona")
	case "darwin":
		if home := os.Getenv("HOME"); home != "" {
			return filepath.Join(home, "Library", "Caches", "NextSona")
		}
		return filepath.Join(os.TempDir(), "NextSona")
	default:
		if xdgCache := os.Getenv("XDG_CACHE_HOME"); xdgCache != "" {
			return filepath.Join(xdgCache, "NextSona")
		}
		if home := os.Getenv("HOME"); home != "" {
			return filepath.Join(home, ".cache", "NextSona")
		}
		return filepath.Join(os.TempDir(), "NextSona")
	}
}
