/**
 * NextSona AI Vocal Engine - Direct Offscreen Orchestrator
...
// did not make it compatible with the model weights used by NextSona.
...
export class AIVocalManager {
  ...
  async detectWebHardwareInfo(backendType) {
    ...
    if (backend === "webgpu") {
      try {
        ...
        console.debug("[NextSona AI] WebGPU adapter details unavailable:", error);
      }
    }
  ...
  async init() {
    try {
      ...
      this.workletNode = new AudioWorkletNode(this.audioCtx, "nextsona-ai-vocal-processor", {
        numberOfInputs: 1,
        numberOfOutputs: 1,
      ...
      if (this.currentMode !== "bypass") {
        this.loadEngine().catch((err) => {
          console.error("[NextSona AI] Background engine load error:", err);
        });
      }
      ...
      if (this.destroyed) return null;
      console.error("[NextSona AI] Worklet creation failed:", err);
  ...
  async runChunkQueue() {
    ...
    try {
      ...
      if (err?.recoverable === true) {
        ...
        console.warn("[NextSona AI] Recoverable GPU processing fault:", err);
        try {
          await this.requestWebGpuRecovery(reason);
        } catch (recoveryError) {
          console.error("[NextSona AI] WebGPU recovery failed:", recoveryError);
        }
      } else {
        console.error("[NextSona AI] Queue processing error:", err);
      }
    } finally {
      this.isBusy = false;
  ...
  attachWebGpuDeviceLossWatcher() {
    ...
    device.lost.then(info => {
      ...
      this.diagnostics.lastRecoveryStartedAt = Date.now();
      console.warn("[NextSona AI] WebGPU device lost:", info?.reason || info?.message || info);
      this.requestWebGpuRecovery("device-lost").catch(error => {
        console.error("[NextSona AI] Device-loss recovery failed:", error);
      });
    }).catch(error => {
      ...
      this.diagnostics.webGpuDeviceLosses++;
      console.warn("[NextSona AI] WebGPU device-loss watcher rejected:", error);
    });
  }
  ...
  async _loadEngine() {
    ...
    try {
      ...
      try {
        ...
        console.log("[NextSona AI] Loaded SIMD STFT WASM");
      } catch (simdErr) {
        console.warn("[NextSona AI] SIMD WASM failed, falling back to scalar:", simdErr);
        const scalarUrl = chrome.runtime.getURL("modules/ai-vocal/stft_scalar.wasm");
        const wasmBuf = await loadProtectedAsset(scalarUrl);
        const instantiated = await WebAssembly.instantiate(wasmBuf, { env: {} });
        instance = instantiated.instance;
        console.log("[NextSona AI] Loaded Scalar STFT WASM fallback");
      }
      ...
      const configureWebGPU = async () => {
        try {
          ...
          console.warn("[NextSona AI] WebGPU unavailable:", webgpuErr);
        }
        return "";
      };
      ...
      const configureWebGL = async () => {
        try {
          ...
          try {
            ...
            console.warn("[NextSona AI] WebGL 2 failed, falling back to WebGL 1:", e2);
          ...
          console.warn("[NextSona AI] WebGL failed completely, falling back to CPU:", webglErr);
      ...
      if (currentBackend === "cpu" || deviceLabel.includes("SwiftShader")) {
        console.warn("[NextSona AI] Software rendering detected (CPU / SwiftShader)");
        this.isHardwareSlow = true;
        ...
        try {
          ...
          if (cached && cached.deviceLabel === deviceLabel &&
            ...
            console.log("[NextSona AI] Cached GPU benchmark retained as history only");
          }
        } catch (_) {}
      }
      ...
      // once by nextsona-model-builder. Runtime loading only selects the
      ...
      const warmupWithOriginalFallback = async () => {
        try {
          ...
          console.warn("[NextSona AI] Native dilation warmup failed; retrying original graph", error);
      ...
      const fallbackToWebGL = async () => {
        ...
        console.warn("[NextSona AI] WebGPU model path failed; retrying with WebGL");
        if (this.model) {
          try { this.model.dispose(); } catch (_) {}
          this.model = null;
        }
      ...
      const fallbackToWebGPU = async () => {
        ...
        console.warn("[NextSona AI] WebGL model path failed; retrying with WebGPU");
        if (this.model) {
          try { this.model.dispose(); } catch (_) {}
          this.model = null;
        }
      ...
      if (modelLoader.foldedCount || modelLoader.explicitPadCount) {
        console.log(`[NextSona AI] Optimized model graph: removed ${modelLoader.foldedCount * 2} data-reordering nodes and ${modelLoader.explicitPadCount} standalone padding nodes (unchanged weights)`);
      }
      ...
      try {
        ...
        console.log(`[NextSona AI] ${currentBackend.toUpperCase()} pipeline pre-warmed`);
      } catch (warmErr) {
      ...
      try {
        ...
        console.log(`[NextSona AI] Hardware benchmark 1-chunk: ${benchmarkMs}ms on ${this.backendName}`);
        ...
        console.warn("[NextSona AI] Benchmark test error:", benchErr);
      }
      ...
      if (powerModeReloadNeeded) {
        ...
        if (!this.powerModeSwitchPromise) {
          this.requestPowerModeReload().catch((error) => {
            console.warn("[NextSona AI] Deferred power mode reload failed:", error);
          });
        }
        return;
      }
      ...
      this.engineLoading = false;
      console.error("[NextSona AI] Engine load failed:", err);
      this.lastError = err.message || err.toString();
  ...
  async processChunk(chunkIndex, rawL, rawR, mode, strength = 1.0, generation = this.streamGeneration) {
    ...
    try {
      ...
      console.error("[NextSona AI] processChunk error:", err);
      this.lastError = err.message || err.toString();
      this.setStatus("ERR: " + this.lastError.substring(0, 16));
    }
  }
  ...
  unloadEngine() {
    ...
    console.log("[NextSona AI] Model unloaded & GPU memory freed");
  }
  ...
  setEngineType(type) {
    ...
    this.streamChunkFloor = null;
    console.log("[NextSona AI] Switched engine to:", this.engineType);
    if (this.workletNode) {
      this.workletNode.port.postMessage({
        type: "SET_ENGINE",
        engineType: this.engineType,
