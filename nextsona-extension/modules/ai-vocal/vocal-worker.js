/**
 * NextSona AI Vocal Engine - Hyper-Optimized Extension Worker
 * 
...
try {
  ...
  console.error("[NextSona Worker] Failed to load tf.min.js:", e);
  ...
  try {
    ...
    if (activeBackend === "webgl") {
      try {
        ...
        console.warn("[NextSona AI] WebGL failed in worker, falling back to CPU:", webglErr);
    ...
    console.log(`[NextSona AI] Initialized successfully with backend: ${activeBackend.toUpperCase()}`);
    self.postMessage({ type: "READY", backend: activeBackend });
    self.postMessage({ type: "STATUS", status: "READY" });
  } catch (err) {
    console.error("[NextSona Worker] Init error:", err);
...
async function processChunk(chunkIndex, rawL, rawR, mode, strength = 1.0, generation = 0) {
  ...
  try {
    ...
    console.error("[NextSona Worker] processChunk error:", err);
    self.postMessage({
      type: "CHUNK_PROCESSED",
      chunkIndex,
      generation,
