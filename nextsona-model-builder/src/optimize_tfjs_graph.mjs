...
export const NEXTSONA_OPTIMIZATION_METADATA = "nextsonaModelOptimization";
...
function appendExactOutputHead(artifacts, nodes, options) {
  ...
  const prefix = "NextSona/optimized_output_head";
  ...
  const allConstants = [
    ...decoderConstants.map(item => ({ ...item, prefix: "NextSona/roi_decoder_layer" })),
    ...roiConstants.map(item => ({ ...item, prefix: "NextSona/roi_decoder_input" })),
  ...
  const decoderPrefix = "NextSona/roi_decoder_layer";
  const roiPrefix = "NextSona/roi_decoder_input";
...
/**
 * Convert the verified converter output into the graph shipped by NextSona.
 *
...
export function buildRuntimeTfjsArtifacts(modelJson, weightBytes) {
  ...
  const runtimeJson = {
    ...
    userDefinedMetadata: {
      ...
      [NEXTSONA_OPTIMIZATION_METADATA]: metadata
    },
    weightsManifest: [{
      ...groups[0],
